...
public class AddYogaActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (role.equals("edit")){
            setDataToEdit(yogaModal);
        }


    }

    private void setDataToEdit(YogaModal yogaModal) {

        dNameEdt.setText(yogaModal.getYogaName());
        dCauseEnEdt.setText(yogaModal.getImpE());
        dCauseHiEdt.setText(yogaModal.getImpH());
        dCureEnEdt.setText(yogaModal.getHowE());
        dCureHiEdt.setText(yogaModal.getHowH());
        dRegimenEnEdt.setText(yogaModal.getNotE());
        dRegimenHiEdt.setText(yogaModal.getNotH());
        videoLinkEdt.setText(yogaModal.getVid());
        Picasso.get().load(yogaModal.getIconUrl()).into(imageView);
        imageView.setVisibility(View.VISIBLE);
        imgLink = yogaModal.getIconUrl();
    ...
    private void getDataAndUpload() {
        ...
        if (causeE.isEmpty()){
            dCauseEnEdt.setError("Benefit is empty !");
            flag = false;
        }
        if (causeH.isEmpty()){
            dCauseHiEdt.setError("Benefit is empty");
            flag = false;
        }
        if (cureE.isEmpty()){
            dCureEnEdt.setError("How to do is empty");
            flag = false;
        }
        if (cureH.isEmpty()){
            dCureHiEdt.setError("How to do is empty");
            flag = false;
        }
        if (regE.isEmpty()){
            dCureEnEdt.setError("Not to do is empty");
            flag = false;
        }
        if (regH.isEmpty()){
    ...
    private void updateToDatabase(String name, String benefitE, String benefitH, String howE, String howH, String regE, String regH, String imgLink, String videoLink) {

        YogaModal yogaModal = new YogaModal();
        yogaModal.setYogaName(name);
        yogaModal.setImpE(benefitE);
        yogaModal.setImpH(benefitH);
        yogaModal.setHowE(howE);
        yogaModal.setHowH(howH);
        yogaModal.setNotE(regE);
        yogaModal.setNotH(regH);
        yogaModal.setIconUrl(imgLink);
        yogaModal.setVid(videoLink);
        yogaModal.setCategory("all");

        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("Yoga");
        databaseReference.child(name).setValue(yogaModal).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {

                Toast.makeText(AddYogaActivity.this, "Added Successfully !", Toast.LENGTH_SHORT).show();
            ...
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(AddYogaActivity.this, "Try again ! "+e, Toast.LENGTH_SHORT).show();
            }
        });
